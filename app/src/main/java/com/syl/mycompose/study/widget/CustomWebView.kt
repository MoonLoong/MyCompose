...
import com.syl.mycompose.study.log.TAG_WEB_VIEW
...
class CustomWebViewClient(private val onEvent: ((WebViewEvent) -> Unit)? = null) : WebViewClient() {
    ...
    override fun shouldOverrideUrlLoading(view: WebView, request: WebResourceRequest): Boolean {
        Timber.tag(TAG_WEB_VIEW).d("url:${request.url},method:${request.method}")
...
fun CustomWebView(
    ...
    AndroidView(factory = {
        Timber.tag(TAG_WEB_VIEW).d("初始化容器")
    ...
    DisposableEffect(Unit) {
        val observer = object : DefaultLifecycleObserver {
            ...
            override fun onPause(owner: LifecycleOwner) {
                Timber.tag(TAG_WEB_VIEW).d("生命周期：onPause")
                agentWeb?.webLifeCycle!!.onPause()
            }

            override fun onResume(owner: LifecycleOwner) {
                Timber.tag(TAG_WEB_VIEW).d("生命周期：onResume")
...
//                Timber.tag(TAG_WEB_VIEW).d("生命周期：onDestroy")
//                agentWeb?.webLifeCycle!!.onDestroy()
//            }
        }
        Timber.tag(TAG_WEB_VIEW).d("开始监听")
        lifecycleOwner.lifecycle.addObserver(observer)
        onDispose {
            Timber.tag(TAG_WEB_VIEW).d("结束监听")
